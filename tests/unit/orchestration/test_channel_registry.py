...
3. *Writer sweep* — every direct subscript write to a boundary key inside
   ``orchestration/nodes/**`` must carry a recorded disposition, so a write into the
...
_NODES_DIR = _REPO_ROOT / "src" / "film_pipeline" / "orchestration" / "nodes"
...
# Every direct subscript write to a boundary key under orchestration/nodes/** must be
...
class TestShotBibleBriefReachesUpdates:
    ...
    def test_update_carries_execution_brief(self) -> None:
        from film_pipeline.orchestration.nodes.visual import shot_bible_node
