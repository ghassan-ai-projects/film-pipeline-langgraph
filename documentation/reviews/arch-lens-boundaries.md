...
- Lazy cross-package imports total **198**: mcp 109, graph 42, app 24, post 10, cli 5, artifacts 3, config 2, generation 2, tui 1.
  *Counting convention (so an independent verifier reproduces 198, not 194): every function-body `import` statement whose target top-level package differs from the enclosing file's, counting **both** `from film_pipeline.x import …` and plain `import film_pipeline.x`, one per statement node. A `from`-only scan undercounts to 194 by omitting exactly four plain-`import` sites — `app/_graph_exec.py:58,168,288` (`import film_pipeline.graph.nodes`) and `cli/driver.py:62` (`import film_pipeline.app.runtime`).* Notable clusters: `graph/nodes/qc.py` (14, mostly validators — see 5d), `mcp/tools/**` (109 — mix of cycle avoidance and deferred-cost style), `app/_graph_exec.py` auto_checkpoint lazily importing schemas (:97-99), `artifacts/matrix_projection.py` lazily importing schemas (:59-69).
