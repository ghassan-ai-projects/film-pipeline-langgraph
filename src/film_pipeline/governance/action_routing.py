...
def _blocking_failure_result(
    state: dict[str, Any], phase: str, facts: GateFacts
) -> RouterResult | None:
    """Rule 2: blocking failure decision → escalate or continue unrelated work."""
    if not facts.has_blocking_failure(state):
        return None
    latest_failure = facts.latest_failure_decision(state)
...
def _blocked_providers_result(
    state: dict[str, Any], phase: str, facts: GateFacts
) -> RouterResult | None:
    """Rule 3: provider-blocked generation → continue unrelated work."""
    blocked_providers = facts.blocked_providers(state)
...
def _pending_revision_result(state: dict[str, Any], facts: GateFacts) -> RouterResult | None:
    """Rule 7: pending revision → revise before anything else."""
    if not facts.has_pending_revision(state):
        return None
    return RouterResult(
...
def _advance_result(
    state: dict[str, Any], phase: str, result: RouterResult, facts: GateFacts
) -> RouterResult:
...
def compute_actions(state: dict[str, Any], facts: GateFacts) -> RouterResult:
    ...
    if routed is not None:
        return routed
    routed = _blocking_failure_result(state, phase, facts)
    if routed is not None:
        return routed
    ...
    if routed is not None:
        return routed
    routed = _pending_revision_result(state, facts)
    if routed is not None:
        return routed
    ...
    return _advance_result(state, phase, result, facts)
