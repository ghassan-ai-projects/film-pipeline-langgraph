...
def check_staleness(
    ...
    # The approved-ref registry lives in orchestrator state (`orchestration`,
    # L9). `governance` sits below it (L8) and must not import it, so the caller
    # supplies the mapping; an omitted mapping means "nothing approved yet".
    return _staleness_warnings(parsed.artifact_id, artifact_ref, built_from, approved_refs or {})
...
def check_phase_consistency(
    ...
    for ref in artifact_refs:
        all_warnings.extend(check_staleness(ref, state, store, approved_refs))
