...
def project_dir(project_slug: str, root: Path = ROOT) -> Path:
    return root / project_slug


def phase_dir(project_slug: str, phase: str, root: Path = ROOT) -> Path:
    return project_dir(project_slug, root) / _PHASE_DIR_MAP.get(phase, phase)


def artifact_dir(project_slug: str, phase: str, artifact_id: str, root: Path = ROOT) -> Path:
    safe_id = artifact_id.replace(":", "_").replace("/", "_")
    return phase_dir(project_slug, phase, root) / safe_id


def artifact_path(
    project_slug: str, phase: str, artifact_id: str, version: int, root: Path = ROOT
) -> Path:
    return artifact_dir(project_slug, phase, artifact_id, root) / "versions" / f"v{version:03}.json"


def current_artifact_path(
    project_slug: str, phase: str, artifact_id: str, root: Path = ROOT
) -> Path:
    return artifact_dir(project_slug, phase, artifact_id, root) / "current.json"


def generated_asset_dir(project_slug: str, scene_id: str, shot_id: str, root: Path = ROOT) -> Path:
    return phase_dir(project_slug, "generation", root) / "scenes" / scene_id / shot_id


def reference_dir(project_slug: str, ref_type: str, root: Path = ROOT) -> Path:
    return project_dir(project_slug, root) / "references" / ref_type


def version_dir(project_slug: str, root: Path = ROOT) -> Path:
    return project_dir(project_slug, root) / "versions"


def checkpoint_dir(project_slug: str, root: Path = ROOT) -> Path:
    return version_dir(project_slug, root) / "checkpoints"


def state_dir(project_slug: str, root: Path = ROOT) -> Path:
