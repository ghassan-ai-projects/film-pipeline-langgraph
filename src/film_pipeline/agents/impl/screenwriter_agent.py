...
class ScreenwriterAgent(BaseAgent):
    ...
    def execute(self, model_output: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            story_bible = _build_story_bible(data.get("story_bible", data))
            script = _build_script(data.get("script", data))
...
def _parse_logline(bible_data: dict[str, Any]) -> Logline:
    """Parse the logline, tolerating both dict and plain-text shapes."""
    ...
    try:
        return Logline(text=logline_text, hook=logline_hook)
    except Exception:
        return Logline(text=logline_text or "Untitled film project.")


def _parse_premise(bible_data: dict[str, Any]) -> Premise:
    """Parse the premise, tolerating both dict and plain-text shapes."""
...
def _parse_act_map(bible_data: dict[str, Any]) -> ActMap:
    """Parse the three-act structure from raw act-map fields."""
...
def _parse_scene_intents(bible_data: dict[str, Any]) -> list[SceneIntent]:
    """Parse scene intents, unwrapping SceneList-shaped dicts."""
...
def _build_story_bible(bible_data: dict[str, Any]) -> StoryBible:
    """Assemble the StoryBible from its parsed sub-models."""
    logline = _parse_logline(bible_data)
    premise = _parse_premise(bible_data)
    act_map = _parse_act_map(bible_data)
    treatment = Treatment(
    ...
    return StoryBible(
        ...
        treatment=treatment,
        act_map=act_map,
        scene_list=SceneList(scenes=_parse_scene_intents(bible_data)),
        setup_payoff_map=[
            SetupPayoffEntry(
...
def _build_dialogue_line(entry: dict[str, Any]) -> DialogueLine:
    """Build one DialogueLine entry from a raw mapping."""
    return DialogueLine(
...
def _build_script(script_data: dict[str, Any]) -> Script:
    """Assemble the Script with computed scene and dialogue totals."""
    script_scenes = [
        ScriptScene(
            ...
            dialogue=[_build_dialogue_line(d) for d in s.get("dialogue", [])],
